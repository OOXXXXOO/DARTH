...
class downloader(Thread):
    ...
    def add_cord(self, left, top, right, bottom, zoom, style='s'):
        ...
        self.wsg_cord = (left_top_x, left_top_y,
                         right_bottom_x, right_bottom_y)

        self.mercator_cord = (*wgs_to_mercator(left_top_x, left_top_y),
                              *wgs_to_mercator(right_bottom_x, right_bottom_y))

        print(
            '# -----WGS BoundingBox: \033[1;32m%s \033[0m' % str(self.wsg_cord))
        print(
            "# -----Mercator BoudingBox: \033[1;32m%s \033[0m" % str(self.mercator_cord))
        # ------------------------------------ III ----------------------------------- #

        self.x_step_point = np.arange(
            left_top_x, right_bottom_x, self.resolution_x*Tilesize)
        self.y_step_point = np.arange(
            left_top_y, right_bottom_y, self.resolution_y*Tilesize)
        ...
        print(
            "# -----Url Queue size: \033[1;32m%s \033[0m" % self.urls_queue.qsize())
    ...
    def downloadurl(self, urls_queue):
        ...
        while (err < 3):
            try:
                ...
                name = "{server}-{x}-{y}-{z}".format(
                    x=info[0], y=info[1], z=info[2], server=self.server)
    ...
    def download(self, output_path="./images"):
        ...
        for index, url in enumerate(self.urls):
            result = pool.apply_async(self.downloadurl, args=(
                self.urls_queue,), callback=update)
    ...
    def get_urls(self, x1, y1, x2, y2, z, style):
        ...
        print(
            "# -----Total tiles number：\033[1;32m{x} X {y}\033[0m".format(x=self.lenx, y=self.leny))
    ...
    def merge(self, path="./result.tif"):
        ...
        output_array = Image.new(
            "RGBA", (self.lenx*Tilesize, self.leny*Tilesize))
        assert not (self.result == None or len(self.result) ==
                    0), "Invalide result of process please check the result of download "+str(self.result)
        ...
        channels=[np.array(i) for i in channels]
        gt = (
            self.extent['LT'][0],
            (self.extent['RB'][0] - self.extent['LT'][0]) /
            channels[0].shape[1],
            0,
            self.extent['LT'][1],
            0,
            (self.extent['RB'][1] - self.extent['LT'][1]) /
            channels[0].shape[0]
        )
        if len(channels) == 3:
